...
export default ()=> {
  ...
  const Text = () => Object.keys(inintialDocs.renderTestDocs).map((item, index) => (
    <button onClick={() => setCurr(inintialDocs.renderTestDocs[item])} key={index}>{item}</button>
  ));
  
  const EditorComponent = ({ doc }) => {
    return (
    <Editor
