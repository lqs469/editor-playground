...
export default function insertRight(editor, opts) {
  ...
  const matchRows = [...Editor.elements(editor, {
    at: [],
    match: { type: defaultOptions.typeRow },
  })];
  ...
  matchRows.forEach(([, rowPath]) => {
    ...
    const [[matchCell, path]] = Editor.nodes(editor, {
      at,
      match: { type: defaultOptions.typeCell },
    });

    path[path.length - 1]++;

    const newCell = createCell(opts, '');
    Editor.insertNodes(editor, newCell, {
      at: path,
      match: {key: matchCell.key },
    });
  });
}
